...
export default {
    ...
    const resp = await axios.get('../../api/devices/'+id);
    this.form.name = resp.data.name;
    this.form.price = resp.data.price;
    this.form.category = resp.data.dcategory_id;
    this.form.technology = resp.data.technology;
    this.form.loan = resp.data.interest;
    this.form.interest_perc = resp.data.interest_perc;
    this.form.total = resp.data.total;
    this.form.insurance = resp.data.insurance;
    this.form.insurance_perc = resp.data.insurance_perc;
    ...
    methods: {
       ...
       menuclick (value) {
                ...
                priceChange(){
            this.form.loan = parseInt(this.form.price)*(parseInt(this.form.interest_perc)/100);
            ...
            try{
                const response = await axios.post('../../api/devices/'+this.form.id,this.form);
                if(response.status == 200){
                    this.loading=false;

                          this.$toast.open({
                            message: 'Device successful edited',
                            type: 'success',
                            duration: 5000,
                            position: 'top-right'
